...
"use strict";
const bedrock$5 = createAmazonBedrock({
...
- When diagnosing crop stress or unusual conditions, query the knowledge base first`,
  model: bedrock$5("us.anthropic.claude-sonnet-4-5-20250929-v1:0"),
...
const secretaryEmbeddingModel = bedrock$3.embedding("amazon.nova-embed-text-v2:0");
...
const ReasonOutputSchema = z.object({
  ...
  actions: z.array(ActionSchema$1).describe("List of parameter changes, harvests, and replants to apply")
});
...
const SENSITIVITY = {
  ...
  lena: { baseSleep: 5.9, sleepVariance: 0.6, stressResilience: 0.5, moraleInertia: 0.4, calorieTarget: 2e3, baseHR: 74, baseBPSys: 115, baseBPDia: 72, baseTemp: 36.9 },
...
function dailyNoise(sol, id) {
  let h = 0;
  const s = `${sol}:${id}`;
  for (let i = 0; i < s.length; i++) h = (h << 5) - h + s.charCodeAt(i) | 0;
  return (h & 2147483647) % 1e3 / 1e3;
}
function deriveStress(stressScore) {
  if (stressScore >= 70) return "good";
  if (stressScore >= 40) return "moderate";
  return "low";
}
function deriveHealth(morale, nutrition, hydration, sleep, stressScore) {
...
function deriveCondition(sleep, stressScore, morale) {
...
function deriveTaskLoad(stressScore, dustStormActive, energyDeficit) {
  let load = 50;
  if (dustStormActive) load += 20;
  if (energyDeficit) load += 15;
  if (stressScore < 40) load += 10;
  if (load >= 70) return "low";
  if (load >= 45) return "moderate";
  return "good";
}
function pickTask(id, env, noise) {
  const pool = TASKS[id];
...
function updateCrew(crew, env, dtHours) {
  if (dtHours <= 0) return crew;
  const stormStress = env.dustStormActive ? (1 - env.dustStormFactor) * 30 : 0;
  const foodStress = env.foodReservesSols < 100 ? (1 - env.foodReservesSols / 100) * 25 : 0;
  ...
  const envStressTotal = stormStress + foodStress + energyStress + co2Stress + o2Stress;
  ...
  const next = crew.map((c) => {
    ...
    const noise = dailyNoise(sol, c.id);
    ...
    const sleepTarget = sens.baseSleep - (envStressTotal > 40 ? 1 : envStressTotal > 20 ? 0.4 : 0) + (noise - 0.5) * sens.sleepVariance;
    const sleep = clamp(
      c.sleep + (sleepTarget - c.sleep) * dtSol * 0.5,
      3.5,
      9.5
    );
    const nutTarget = nutCov * 100 * (sens.calorieTarget / 2500);
    ...
    const calories = Math.round(
      sens.calorieTarget * clamp(nutCov * 1.1, 0.5, 1.1) + (noise - 0.5) * 100
    );
    const moraleTarget = nutrition * 0.25 + hydration * 0.15 + stressScore * 0.3 + sleep / 8 * 100 * 0.15 + (env.foodReservesSols > 200 ? 15 : env.foodReservesSols > 100 ? 8 : 0);
    const morale = clamp(
      c.morale + (moraleTarget - c.morale) * dtSol * (1 - sens.moraleInertia) * 2,
      5,
      100
    );
    const stressDelta = (100 - stressScore) / 100;
    ...
    if (c.morale === Math.round(morale) && c.nutrition === Math.round(nutrition) && c.hydration === Math.round(hydration) && c.sleep === Math.round(sleep * 10) / 10 && c.stress === stress && c.health === health && c.heartRate === heartRate && c.bloodPressure === bloodPressure && c.bodyTemp === bodyTemp && c.o2Sat === o2Sat && c.condition === condition && c.calories === calories && c.taskLoad === taskLoad && c.currentTask === currentTask && c.evaHours === evaHours) {
      return c;
    }
    changed = true;
    return {
