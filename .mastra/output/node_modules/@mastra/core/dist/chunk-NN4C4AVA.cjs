...
var AIV5Adapter = class {
  ...
  static fromUIMessage(uiMsg) {
    ...
    const toolInvocationParts = parts.filter((p) => chunkVBVOKSJK_cjs.isToolUIPart(p));
    ...
    const v2Parts = parts.map((p) => {
      if (chunkVBVOKSJK_cjs.isToolUIPart(p)) {
        const toolName = getToolName(p);
...
var CacheKeyGenerator = class _CacheKeyGenerator {
  ...
  static fromAIV5Parts(parts) {
    ...
    for (const part of parts) {
      ...
      if (chunkVBVOKSJK_cjs.isToolUIPart(part) || part.type === "dynamic-tool") {
        key += part.toolCallId;
        key += part.state;
      }
      if (part.type === "reasoning") {
  ...
  const msgs = messages.map((m) => {
    ...
    const sanitized = {
      ...
      parts: safeParts.map((part) => {
        ...
        if (chunkVBVOKSJK_cjs.isToolUIPart(part) && part.state === "output-available") {
          return {
            ...part,
...
function addStartStepPartsForAIV5(messages) {
  for (const message of messages) {
    ...
    for (const [index, part] of message.parts.entries()) {
      if (!chunkVBVOKSJK_cjs.isToolUIPart(part)) continue;
      const nextPart = message.parts.at(index + 1);
      if (nextPart && nextPart.type !== `step-start` && !chunkVBVOKSJK_cjs.isToolUIPart(nextPart)) {
...
function aiV5UIMessagesToAIV5ModelMessages(messages, dbMessages, filterIncompleteToolCalls = false) {
  ...
  const result = chunkVBVOKSJK_cjs.convertToModelMessages(preprocessed);
...
var StepContentExtractor = class _StepContentExtractor {
  ...
  static convertPartsToContent(parts, stepId, stepContentFn) {
    ...
    const modelMessages = chunkVBVOKSJK_cjs.convertToModelMessages(sanitizeV5UIMessages(stepUiMessages));
...
//# sourceMappingURL=chunk-NN4C4AVA.cjs.map
//# sourceMappingURL=chunk-NN4C4AVA.cjs.map