...
var StepExecutor = class extends chunkRO47SMI7_cjs.MastraBase {
  ...
  async execute(params) {
    ...
    const { inputData, validationError } = await chunkFSGRQN2S_cjs.validateStepInput({
    ...
    try {
      ...
      const stepOutput = await step.execute(
        chunkFSGRQN2S_cjs.createDeprecationProxy(
          {
            ...
            getStepResult: chunkFSGRQN2S_cjs.getStepResult.bind(this, stepResults),
            suspend: async (suspendPayload, suspendOptions) => {
              const { suspendData, validationError: validationError2 } = await chunkFSGRQN2S_cjs.validateStepSuspendData({
            ...
            ...chunkLIRWMGGI_cjs.createObservabilityContext(params.tracingContext)
          },
          {
            paramName: "runCount",
            deprecationMessage: chunkFSGRQN2S_cjs.runCountDeprecationMessage,
            logger: this.logger
          }
        )
      );
      ...
      return {
        ...
        tripwire: error instanceof chunkFSGRQN2S_cjs.TripWire ? {
      ...
      chunkFSGRQN2S_cjs.createDeprecationProxy(
        {
          ...
          abortSignal: abortController?.signal,
          // TODO
          ...chunkLIRWMGGI_cjs.createObservabilityContext(),
          iterationCount
        },
        {
          paramName: "runCount",
...
function createTripWireFromChunk(chunk) {
  const { payload } = chunk;
  return new chunkFSGRQN2S_cjs.TripWire(
  ...
  if (resumeData !== void 0 && forEachIndex === void 0 && currentResult?.output?.length > 0) {
    ...
    if (suspendedIndices.length > 0) {
      ...
      for (const suspIdx of indicesToResume) {
        updatedOutput[suspIdx] = chunkK5H3MT3B_cjs.createPendingMarker();
      }
  ...
  async processWorkflowStepRun({
    ...
    if (step.step instanceof EventedWorkflow || step.step.component === "WORKFLOW") {
      if (resumeSteps?.length === 1 && resumeSteps[0] === step.step.id) {
        ...
        const timeTravelParams = chunkFSGRQN2S_cjs.createTimeTravelExecutionParams({
    ...
    const { resumeData: timeTravelResumeData, validationError: timeTravelResumeValidationError } = await chunkFSGRQN2S_cjs.validateStepResumeData({
...
function cloneWorkflow(workflow, opts) {
  const wf = new chunkFSGRQN2S_cjs.Workflow({
    id: opts.id,
...
function isAgent(input) {
  return input instanceof chunkFSGRQN2S_cjs.Agent;
}
function isToolStep(input) {
  return input instanceof chunkNQXA2BUF_cjs.Tool;
}
function isStepParams(input) {
  return input !== null && typeof input === "object" && "id" in input && "execute" in input && !(input instanceof chunkFSGRQN2S_cjs.Agent) && !(input instanceof chunkNQXA2BUF_cjs.Tool);
}
function isProcessor(obj) {
  return obj !== null && typeof obj === "object" && "id" in obj && typeof obj.id === "string" && !(obj instanceof chunkFSGRQN2S_cjs.Agent) && !(obj instanceof chunkNQXA2BUF_cjs.Tool) && (typeof obj.processInput === "function" || typeof obj.processInputStep === "function" || typeof obj.processOutputStream === "function" || typeof obj.processOutputResult === "function" || typeof obj.processOutputStep === "function");
}
...
async function processAgentStream(params) {
  ...
  for await (const chunk of fullStream) {
    ...
    if (streamFormat !== "legacy") {
      await chunkFSGRQN2S_cjs.forwardAgentStreamChunk({ writer, chunk });
    }
  }
  try {
    await pubsub.publish(`workflow.events.v2.${runId}`, {
...
function createStepFromAgent(params, agentOrToolOptions) {
  ...
  return {
    ...
    execute: async ({
      ...
      const observabilityContext = chunkLIRWMGGI_cjs.resolveObservabilityContext(obsFields);
      const logger = mastra?.getLogger();
      const toolData = {
        name: params.name,
        args: inputData
      ...
      const isV2Model = chunkFSGRQN2S_cjs.isSupportedLanguageModel(modelInfo);
...
function createStepFromProcessor(processor) {
  ...
  return {
    ...
    inputSchema: chunkVWJ7H3MJ_cjs.toStandardSchema(chunkFSGRQN2S_cjs.ProcessorStepSchema),
    outputSchema: chunkVWJ7H3MJ_cjs.toStandardSchema(chunkFSGRQN2S_cjs.ProcessorStepOutputSchema),
    execute: async ({ inputData, requestContext, ...obsFields }) => {
      ...
      const abort = (reason, options) => {
        throw new chunkFSGRQN2S_cjs.TripWire(reason || `Tripwire triggered by ${processor.id}`, options, processor.id);
      };
      ...
      if (processorStates) {
        ...
        if (!ps) {
          ps = new chunkFSGRQN2S_cjs.ProcessorState();
      ...
      const executePhaseWithSpan = async (fn) => {
        try {
          ...
          if (error instanceof chunkFSGRQN2S_cjs.TripWire) {
        ...
        switch (phase) {
          case "input": {
            if (processor.processInput) {
              ...
              if (result instanceof chunkNN4C4AVA_cjs.MessageList) {
                ...
                chunkFSGRQN2S_cjs.ProcessorRunner.applyMessagesToMessageList(
                  result,
          ...
          case "inputStep": {
            if (processor.processInputStep) {
              ...
              const validatedResult = await chunkFSGRQN2S_cjs.ProcessorRunner.validateAndFormatProcessInputStepResult(result, {
                messageList: passThrough.messageList,
                processor,
                stepNumber: stepNumber ?? 0
              });
...
var EventedWorkflow = class extends chunkFSGRQN2S_cjs.Workflow {
  constructor(params) {
    super(params);
...
var EventedRun = class extends chunkFSGRQN2S_cjs.Run {
  constructor(params) {
    super(params);
  ...
  stream({
    ...
    this.streamOutput = new chunkFSGRQN2S_cjs.WorkflowRunOutput({
      runId: this.runId,
      workflowId: this.workflowId,
      stream
    });
...
var EventedExecutionEngine = class extends chunkFSGRQN2S_cjs.ExecutionEngine {
  eventProcessor;
  constructor({
    mastra,
    eventProcessor,
  ...
  async execute(params) {
    ...
    const finishCb = async (event, ack) => {
      ...
      if (["workflow.end", "workflow.fail", "workflow.suspend"].includes(event.type)) {
        ...
        if (event.type === "workflow.fail" && event.data.stepResults) {
          event.data.stepResults = chunkFSGRQN2S_cjs.hydrateSerializedStepErrors(event.data.stepResults);
        }
    ...
    for (const [stepId, stepResult] of Object.entries(stepResultsWithoutTopLevelState)) {
      cleanStepResults[stepId] = chunkFSGRQN2S_cjs.cleanStepResult(stepResult);
    }
    let callbackArg;
...
//# sourceMappingURL=chunk-EQ7RFQAH.cjs.map
//# sourceMappingURL=chunk-EQ7RFQAH.cjs.map