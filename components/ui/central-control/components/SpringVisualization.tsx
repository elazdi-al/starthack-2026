...
export function SpringVisualization({ spring, isSimpleMode }: SpringVisualizationProps) {
  ...
  for (let i = 1; i < 4; i++) {
    ...
    const y = (height / 4) * i;
    gridLines.push(
      <line key={`v-${i}`} x1={x} y1={0} x2={x} y2={height} stroke="var(--dial-border)" strokeWidth="1" />,
      <line key={`h-${i}`} x1={0} y1={y} x2={width} y2={y} stroke="var(--dial-border)" strokeWidth="1" />
    );
  }
  ...
  return (
    <svg viewBox={`0 0 ${width} ${height}`} className="dialkit-spring-viz">
      ...
      <path
        d={pathData}
        fill="none"
        stroke="var(--dial-text-secondary)"
        strokeWidth="2"
        strokeLinecap="round"
