...
var CoreToolBuilder = class extends chunkRO47SMI7_cjs.MastraBase {
  ...
  constructor(input) {
    ...
    if (!chunkNQXA2BUF_cjs.isVercelTool(this.originalTool) && (input.autoResumeSuspendedTools || this.originalTool.id?.startsWith("agent-") || this.originalTool.id?.startsWith("workflow-"))) {
  ...
  getParameters = () => {
    if (chunkNQXA2BUF_cjs.isVercelTool(this.originalTool)) {
  ...
  createExecute(tool, options, logType) {
    ...
    const execFunction = async (args, execOptions) => {
      ...
      const mcpMeta = !chunkNQXA2BUF_cjs.isVercelTool(tool) && "mcpMetadata" in tool ? tool.mcpMetadata : void 0;
      const toolRequestContext = execOptions.requestContext ?? options.requestContext;
      const toolSpan = chunkLIRWMGGI_cjs.getOrCreateSpan({
        ...
        input: args,
        entityType: chunkLIRWMGGI_cjs.EntityType.TOOL,
      ...
      try {
        ...
        if (chunkNQXA2BUF_cjs.isVercelTool(tool)) {
          result = await chunkLIRWMGGI_cjs.executeWithContext({
            span: toolSpan,
            fn: async () => tool?.execute?.(args, execOptions)
          });
        } else {
          const wrappedMastra = options.mastra ? chunkLIRWMGGI_cjs.wrapMastra(options.mastra, { currentSpan: toolSpan }) : options.mastra;
          ...
          const baseContext = {
            ...
            ...chunkLIRWMGGI_cjs.createObservabilityContext({ currentSpan: toolSpan }),
          ...
          if (resumeData) {
            const resumeValidation = chunkNQXA2BUF_cjs.validateToolInput(resumeSchema, resumeData, options.name);
          ...
          result = await chunkLIRWMGGI_cjs.executeWithContext({ span: toolSpan, fn: async () => tool?.execute?.(args, toolContext) });
        }
        if (suspendData) {
          const suspendSchema = this.getSuspendSchema();
          const suspendValidation = chunkNQXA2BUF_cjs.validateToolSuspendData(suspendSchema, suspendData, options.name);
        ...
        if (chunkNQXA2BUF_cjs.isVercelTool(tool)) {
          ...
          const outputValidation = chunkNQXA2BUF_cjs.validateToolOutput(outputSchema, result, options.name, false);
    ...
    return async (args, execOptions) => {
      ...
      try {
        ...
        const { data, error: error2 } = chunkNQXA2BUF_cjs.validateToolInput(parameters, args, options.name);
  ...
  build() {
    ...
    let needsApprovalFn;
    if (chunkNQXA2BUF_cjs.isVercelTool(this.originalTool) && "needsApproval" in this.originalTool) {
...
function ensureToolProperties(tools) {
  const toolsWithProperties = Object.keys(tools).reduce((acc, key) => {
    ...
    if (tool) {
      if (typeof tool === "function" && !(tool instanceof chunkNQXA2BUF_cjs.Tool) && !chunkNQXA2BUF_cjs.isVercelTool(tool)) {
...
//# sourceMappingURL=chunk-DDIY3CRP.cjs.map
//# sourceMappingURL=chunk-DDIY3CRP.cjs.map