...
var chunkDDIY3CRP_cjs = require('./chunk-DDIY3CRP.cjs');
...
var StructuredOutputProcessor = class {
  ...
  async processOutputStream(args) {
    ...
    const observabilityContext = chunkLIRWMGGI_cjs.resolveObservabilityContext(rest);
...
var ProcessorState = class {
  ...
  constructor(options) {
    ...
    this.span = parentSpan?.createChildSpan({
      ...
      entityType: chunkLIRWMGGI_cjs.EntityType.OUTPUT_PROCESSOR,
...
var ProcessorRunner = class _ProcessorRunner {
    ...
    for (const [index, processorOrWorkflow] of this.outputProcessors.entries()) {
      ...
      try {
        ...
        const processResult = await processMethod({
          ...
          abort,
          ...chunkLIRWMGGI_cjs.createObservabilityContext({ currentSpan: processorSpan }),
          requestContext,
          retryCount,
          writer
        });
        const mutations = messageList.stopRecording();
        if (processResult instanceof chunkNN4C4AVA_cjs.MessageList) {
        ...
        try {
          if (processor.processOutputStream && processedPart) {
            ...
            const result = await processor.processOutputStream({
              ...
              ...chunkLIRWMGGI_cjs.createObservabilityContext({ currentSpan: state.span }),
              requestContext,
              messageList,
              retryCount,
              writer
  ...
  async runInputProcessors(messageList, observabilityContext, requestContext, retryCount = 0) {
    for (const [index, processorOrWorkflow] of this.inputProcessors.entries()) {
      ...
      const processorSpan = parentSpan?.createChildSpan({
        ...
        entityType: chunkLIRWMGGI_cjs.EntityType.INPUT_PROCESSOR,
        entityId: processor.id,
        entityName: processor.name,
        attributes: {
          processorExecutor: "legacy",
      ...
      try {
        ...
        let mutations;
        if (result instanceof chunkNN4C4AVA_cjs.MessageList) {
  ...
  async runProcessInputStep(args) {
    ...
    const observabilityContext = chunkLIRWMGGI_cjs.resolveObservabilityContext(args);
    const stepInput = {
    ...
    for (const [index, processorOrWorkflow] of processors.entries()) {
      ...
      const processorSpan = currentSpan?.createChildSpan({
        ...
        entityType: chunkLIRWMGGI_cjs.EntityType.INPUT_STEP_PROCESSOR,
        entityId: processor.id,
        entityName: processor.name,
        attributes: {
          processorExecutor: "legacy",
  ...
  async runProcessOutputStep(args) {
    ...
    for (const [index, processorOrWorkflow] of this.outputProcessors.entries()) {
      ...
      const processorSpan = parentSpan?.createChildSpan({
        ...
        entityType: chunkLIRWMGGI_cjs.EntityType.OUTPUT_STEP_PROCESSOR,
        entityId: processor.id,
        entityName: processor.name,
        attributes: {
          processorExecutor: "legacy",
...
var ObjectFormatHandler = class extends BaseFormatHandler {
  ...
  async processPartialChunk({
    ...
    const { value: currentObjectJson, state } = await chunkVBVOKSJK_cjs.parsePartialJson(processedAccumulatedText);
    if (this.validatePartialChunks && this.partialSchema) {
      const result = this.partialSchema?.safeParse(currentObjectJson);
      if (result.success && result.data && result.data !== void 0 && !chunkVBVOKSJK_cjs.isDeepEqualData(previousObject, result.data)) {
        return {
          shouldEmit: true,
    ...
    if (currentObjectJson !== void 0 && currentObjectJson !== null && typeof currentObjectJson === "object" && !chunkVBVOKSJK_cjs.isDeepEqualData(previousObject, currentObjectJson)) {
      return {
  ...
  async validateAndTransformFinal(finalRawValue) {
    ...
    const { value } = await chunkVBVOKSJK_cjs.parsePartialJson(rawValue);
...
var ArrayFormatHandler = class extends BaseFormatHandler {
  ...
  async processPartialChunk({
    ...
    const processedAccumulatedText = this.preprocessText(accumulatedText);
    const { value: currentObjectJson, state: parseState } = await chunkVBVOKSJK_cjs.parsePartialJson(processedAccumulatedText);
    if (currentObjectJson !== void 0 && !chunkVBVOKSJK_cjs.isDeepEqualData(previousObject, currentObjectJson)) {
      ...
      if (!chunkVBVOKSJK_cjs.isDeepEqualData(this.textPreviousFilteredArray, filteredElements)) {
...
var EnumFormatHandler = class extends BaseFormatHandler {
  ...
  async processPartialChunk({
    ...
    const processedAccumulatedText = this.preprocessText(accumulatedText);
    const { value: currentObjectJson } = await chunkVBVOKSJK_cjs.parsePartialJson(processedAccumulatedText);
    if (currentObjectJson !== void 0 && currentObjectJson !== null && typeof currentObjectJson === "object" && !Array.isArray(currentObjectJson) && "result" in currentObjectJson && typeof currentObjectJson.result === "string" && !chunkVBVOKSJK_cjs.isDeepEqualData(previousObject, currentObjectJson)) {
  ...
  async validateAndTransformFinal(rawFinalValue) {
    ...
    const { value } = await chunkVBVOKSJK_cjs.parsePartialJson(processedValue);
          ...
          async transform(chunk, controller) {
            if (chunk.type === "finish" && chunk.payload?.stepResult?.reason === "tool-calls") {
              ...
              const {
                ...
                chunk,
                processorStates,
                chunkLIRWMGGI_cjs.resolveObservabilityContext(options),
            ...
            case "finish":
              ...
              try {
                if (self.processorRunner && !self.#options.isLLMExecutionStep) {
                  ...
                  const outputText = lastResponseMessage ? chunkNN4C4AVA_cjs.coreContentToString(lastResponseMessage.content) : "";
  ...
  switch (chunk.type) {
    ...
    case "file":
      if (mode === "generate") {
        return {
          type: "file",
          file: new chunkNN4C4AVA_cjs.DefaultGeneratedFile({
      ...
      return {
        type: "file",
        file: new chunkNN4C4AVA_cjs.DefaultGeneratedFileWithType({
          data: chunk.payload.data,
          mediaType: chunk.payload.mimeType
        })
      };
...
var createTimeTravelExecutionParams = (params) => {
  ...
  for (const [index, entry] of graph.steps.entries()) {
    ...
    stepIds.forEach((stepId) => {
      ...
      if (result) {
        const formattedResult = chunkDDIY3CRP_cjs.removeUndefinedValues(result);
  ...
  const results = await Promise.all(
    entry.steps.map(async (step, i) => {
      ...
      const stepExecResult = await engine.executeStep({
        ...
        ...chunkLIRWMGGI_cjs.createObservabilityContext({ currentSpan: parallelSpan }),
        pubsub,
        abortController,
        requestContext,
        outputWriter,
      ...
      const context = createDeprecationProxy(
        {
          ...
          state: executionContext.state,
          retryCount: -1,
          ...chunkLIRWMGGI_cjs.createObservabilityContext({ currentSpan: evalSpan }),
  ...
  const results = await Promise.all(
    stepsToRun.map(async (step) => {
      ...
      const stepExecResult = await engine.executeStep({
        ...
        ...chunkLIRWMGGI_cjs.createObservabilityContext({ currentSpan: conditionalSpan }),
        pubsub,
        abortController,
        requestContext,
        outputWriter,
...
async function executeLoop(engine, params) {
  ...
  do {
    const stepExecResult = await engine.executeStep({
      ...
      ...chunkLIRWMGGI_cjs.createObservabilityContext({ currentSpan: loopSpan }),
      pubsub,
      abortController,
      requestContext,
      outputWriter,
    ...
    const evalSpan = await engine.createChildSpan({
      ...
      options: {
        ...
        input: chunkDDIY3CRP_cjs.selectFields(result.output, ["stepResult", "output.text", "output.object", "messages"]),
        attributes: {
  ...
  if (fn) {
    ...
    duration = await engine.wrapDurableOperation(`workflow.${workflowId}.sleep.${entry.id}`, async () => {
      return fn({
        ...
        retryCount: -1,
        ...chunkLIRWMGGI_cjs.createObservabilityContext({ currentSpan: sleepSpan }),
        getInitData: () => stepResults?.input,
        getStepResult: getStepResult.bind(null, stepResults),
  ...
  if (fn) {
    ...
    const dateResult = await engine.wrapDurableOperation(`workflow.${workflowId}.sleepUntil.${entry.id}`, async () => {
      return fn({
        ...
        retryCount: -1,
        ...chunkLIRWMGGI_cjs.createObservabilityContext({ currentSpan: sleepUntilSpan }),
        getInitData: () => stepResults?.input,
        getStepResult: getStepResult.bind(null, stepResults),
        // TODO: this function shouldn't have suspend probably?
        suspend: async (_suspendPayload) => {
...
async function executeStep(engine, params) {
  ...
  const stepSpan = await engine.createStepSpan({
    ...
    options: {
      ...
      entityType: chunkLIRWMGGI_cjs.EntityType.WORKFLOW_STEP,
      entityId: step.id,
      input: inputData,
      tracingPolicy: engine.options?.tracingPolicy,
      requestContext
  ...
  const stepRetryResult = await engine.executeStepWithRetry(
    ...
    async () => {
      ...
      const mastraForStep = engine.mastra ? isNestedWorkflow ? engine.mastra : chunkLIRWMGGI_cjs.wrapMastra(engine.mastra, { currentSpan: stepSpan }) : void 0;
      const output = await runStep({
        ...
        ...chunkLIRWMGGI_cjs.createObservabilityContext({ currentSpan: stepSpan }),
        getInitData: () => stepResults?.input,
        getStepResult: getStepResult.bind(null, stepResults),
  ...
  async execute(params) {
    ...
    for (let i = startIdx; i < steps.length; i++) {
      ...
      lastOutput = await this.executeEntry({
        ...
        timeTravel,
        restart,
        ...chunkLIRWMGGI_cjs.createObservabilityContext({ currentSpan: workflowSpan }),
...
function createStepFromProcessor(processor) {
  const getProcessorEntityType = (phase) => {
    switch (phase) {
      case "input":
        return chunkLIRWMGGI_cjs.EntityType.INPUT_PROCESSOR;
      case "inputStep":
        return chunkLIRWMGGI_cjs.EntityType.INPUT_STEP_PROCESSOR;
      case "outputStream":
      case "outputResult":
        return chunkLIRWMGGI_cjs.EntityType.OUTPUT_PROCESSOR;
      case "outputStep":
        return chunkLIRWMGGI_cjs.EntityType.OUTPUT_STEP_PROCESSOR;
      default:
        return chunkLIRWMGGI_cjs.EntityType.OUTPUT_PROCESSOR;
    }
  };
  ...
  return {
    ...
    execute: async ({ inputData, requestContext, tracingContext, outputWriter }) => {
      ...
      const processorObservabilityContext = chunkLIRWMGGI_cjs.createObservabilityContext(
      ...
      const passThrough = {
        ...
        messageList: messageList ?? (Array.isArray(messages) ? new chunkNN4C4AVA_cjs.MessageList().add(messages, "input").addSystem(systemMessages ?? []) : void 0),
        stepNumber,
        systemMessages,
        streamParts,
        state,
        ...
        switch (phase) {
          ...
          case "outputStream": {
            ...
            if (processor.processOutputStream) {
              ...
              const processorObservabilityContext2 = chunkLIRWMGGI_cjs.createObservabilityContext(
...
` + errors.map((e) => {
  ...
  async _start({
    ...
    const observabilityContext = chunkLIRWMGGI_cjs.resolveObservabilityContext(rest);
    const workflowSpan = chunkLIRWMGGI_cjs.getOrCreateSpan({
      type: "workflow_run" /* WORKFLOW_RUN */,
      name: `workflow run: '${this.workflowId}'`,
      entityType: chunkLIRWMGGI_cjs.EntityType.WORKFLOW_RUN,
  ...
  async _resume(params) {
    const observabilityContext = chunkLIRWMGGI_cjs.resolveObservabilityContext(params);
    const workflowsStore = await this.#mastra?.getStorage()?.getStore("workflows");
...
var MastraScorer = class _MastraScorer {
  ...
  async run(input) {
    ...
    const observabilityContext = chunkLIRWMGGI_cjs.resolveObservabilityContext(input);
        ...
        if (inputStepProcessors && inputStepProcessors.length > 0) {
          ...
          try {
            ...
            const processInputStepResult = await processorRunner.runProcessInputStep({
              ...
              ...chunkLIRWMGGI_cjs.createObservabilityContext(stepTracingContext),
              requestContext,
              model,
            ...
            if (processInputStepResult.tools && currentStep.tools) {
              ...
              for (const [name, tool2] of Object.entries(currentStep.tools)) {
                if (chunkNQXA2BUF_cjs.isMastraTool(tool2)) {
                  convertedTools[name] = chunkDDIY3CRP_cjs.makeCoreTool(
                    tool2,
                    {
                      name,
                      runId,
...
Analyse the suspended tools: ${JSON.stringify(suspendedTools)}, using the messages available to you and the resumeSchema of each suspended tool, find the tool whose resumeData you can construct properly.
        ...
        if (isSupportedLanguageModel(currentStep.model)) {
          modelResult = chunkLIRWMGGI_cjs.executeWithContextSync({
      ...
      if (outputProcessors && outputProcessors.length > 0) {
        ...
        try {
          ...
          await processorRunner.runProcessOutputStep({
            ...
            text: immediateText,
            ...chunkLIRWMGGI_cjs.createObservabilityContext(outputStepTracingContext),
            requestContext,
...
function createLLMMappingStep({ models, _internal, ...rest }, llmExecutionStep) {
  ...
  const observabilityContext = chunkLIRWMGGI_cjs.createObservabilityContext(rest.modelSpanTracker?.getTracingContext());
          ...
          for (const toolCall of errorResults) {
            ...
            rest.messageList.updateToolInvocation({
              ...
              toolInvocation: {
                state: "result",
                toolCallId: toolCall.toolCallId,
                toolName: chunkNN4C4AVA_cjs.sanitizeToolName(toolCall.toolName),
                args: toolCall.args,
  ...
  return new web.ReadableStream({
    start: async (controller) => {
      ...
      const executionResult = resumeContext ? await run.resume({
        ...
        ...chunkLIRWMGGI_cjs.createObservabilityContext(rest.modelSpanTracker?.getTracingContext()),
        requestContext,
        label: toolCallId
      }) : await run.start({
...
var MastraLLMVNext = class extends chunkRO47SMI7_cjs.MastraBase {
  ...
  stream({
    resumeContext,
    runId,
    stopWhen = chunkVBVOKSJK_cjs.stepCountIs(5),
    maxSteps,
    tools = {},
    modelSettings,
    toolChoice = "auto",
    ...
    if (maxSteps && typeof maxSteps === "number") {
      stopWhenToUse = chunkVBVOKSJK_cjs.stepCountIs(maxSteps);
    } else {
        ...
        options: {
          ...
          onStepFinish: async (props) => {
            ...
            if (!isNaN(remainingTokens) && remainingTokens > 0 && remainingTokens < 2e3) {
              ...
              await chunkDDIY3CRP_cjs.delay(10 * 1e3);
            }
          },
...
async function safeParseLLMJson(text) {
  ...
  const { value, state } = await chunkVBVOKSJK_cjs.parsePartialJson(preprocessed);
...
async function prepareMemoryStep({
  ...
  if (typeof messages === "string") {
    ...
    const messageList = new chunkNN4C4AVA_cjs.MessageList({
      threadId: thread?.id,
      resourceId: thread?.resourceId
    });
...
async function saveMessagesWithProcessors(memory, messages, processorRunner, context) {
  ...
  await processorRunner.runOutputProcessors(
    messageList,
    chunkLIRWMGGI_cjs.resolveObservabilityContext(observabilityContext),
    requestContext
  );
    ...
    execute: async ({ inputData, getInitData, writer, resumeData, mastra, suspend }) => {
      ...
      const finalResult = await tool2.execute(
        inputDataToUse,
        {
          ...
          ...chunkLIRWMGGI_cjs.createObservabilityContext({ currentSpan: void 0 }),
          writer
        },
        { toolCallId, messages: [] }
      );
        ...
        if (requireApprovalMetadata || suspendedTools) {
          ...
          `;
              ...
              const result = llm.stream({
                ...
                messageList,
                agentId: routingAgent.id,
                ...chunkLIRWMGGI_cjs.resolveObservabilityContext(routingAgentOptions ?? {}),
                structuredOutput: {
                  schema: z9__default.default.object({
...
var AgentLegacyHandler = class {
  ...
  __primitive({
    ...
    return {
      before: async () => {
        ...
        const agentSpan = chunkLIRWMGGI_cjs.getOrCreateSpan({
          type: "agent_run" /* AGENT_RUN */,
          name: `agent run: '${this.capabilities.id}'`,
          entityType: chunkLIRWMGGI_cjs.EntityType.AGENT,
        ...
        const innerObservabilityContext = chunkLIRWMGGI_cjs.createObservabilityContext({ currentSpan: agentSpan });
        ...
        let messageList = new chunkNN4C4AVA_cjs.MessageList({
          threadId,
          resourceId,
      ...
      after: async ({
        ...
        const messageListResponses = new chunkNN4C4AVA_cjs.MessageList({
          threadId,
          resourceId,
          generateMessageId: this.capabilities.mastra?.generateId?.bind(this.capabilities.mastra),
          // @ts-expect-error Flag for agent network messages
        ...
        if (memory && resourceId && thread2) {
          try {
            ...
            if (shouldGenerate && !thread2.title && userMessage) {
              const observabilityContext2 = chunkLIRWMGGI_cjs.createObservabilityContext({ currentSpan: agentSpan });
              promises.push(
        ...
        await this.capabilities.runScorers({
          ...
          threadId,
          resourceId,
          ...chunkLIRWMGGI_cjs.createObservabilityContext({ currentSpan: agentSpan })
        });
        const scoringData = {
          input: {
  ...
  async prepareLLMOptions(messages, options, methodType) {
    ...
    const { before, after } = this.__primitive({
      ...
      methodType,
      tracingOptions,
      ...chunkLIRWMGGI_cjs.resolveObservabilityContext(args)
    });
    let messageList;
    let thread;
    let threadExists;
  ...
  async generateLegacy(messages, generateOptions = {}) {
    ...
    const observabilityContext = chunkLIRWMGGI_cjs.createObservabilityContext({ currentSpan: agentSpan });
...
var prepareMemoryStepOutputSchema = z9.z.object({
  ...
  messageList: z9.z.instanceof(chunkNN4C4AVA_cjs.MessageList),
...
var Agent = class extends chunkRO47SMI7_cjs.MastraBase {
  ...
  constructor(config) {
    ...
    if (config.voice) {
      ...
      this.#voice = new chunkLDLSSJ7X_cjs.DefaultVoice();
    }
    if (config.workspace) {
...
${errorMessages}`,
  ...
  async getVoice({ requestContext } = {}) {
    if (this.#voice) {
      ...
      return new chunkLDLSSJ7X_cjs.DefaultVoice();
    }
  }
  /**
  ...
  listTools({ requestContext = new chunk5WBEMKE2_cjs.RequestContext() } = {}) {
    if (typeof this.#tools !== "function") {
      return chunkDDIY3CRP_cjs.ensureToolProperties(this.#tools);
    }
    ...
    return resolveMaybePromise(result, (tools) => {
      ...
      return chunkDDIY3CRP_cjs.ensureToolProperties(tools);
    });
  }
  /**
    ...
    return modelSelectionPromise.then((modelSelection) => {
      ...
      return resolveMaybePromise(resolvedModel, (modelInfo) => {
        ...
        if (isSupportedLanguageModel(modelInfo)) {
          ...
          llm = new chunkJ7Q3CZI5_cjs.MastraLLMV1({
            model: modelInfo,
            mastra: this.#mastra,
            options: { tracingPolicy: this.#options?.tracingPolicy }
          });
  ...
  async generateTitleFromUserMessage({
    ...
    const normMessage = new chunkNN4C4AVA_cjs.MessageList().add(message, "user").get.all.aiV5.ui().at(-1);
    if (!normMessage) {
    ...
    if (isSupportedLanguageModel(llm.getModel())) {
      const messageList = new chunkNN4C4AVA_cjs.MessageList().add(
        [
          {
            role: "system",
  ...
  async genTitle(userMessage, requestContext, observabilityContext, model, instructions) {
    try {
      if (userMessage) {
        const normMessage = new chunkNN4C4AVA_cjs.MessageList().add(userMessage, "user").get.all.ui().at(-1);
        if (normMessage) {
  ...
  async listMemoryTools({
    ...
    if (memoryTools) {
      ...
      for (const [toolName, tool2] of Object.entries(memoryTools)) {
        ...
        const convertedToCoreTool = chunkDDIY3CRP_cjs.makeCoreTool(toolObj, options, void 0, autoResumeSuspendedTools);
  ...
  async listWorkspaceTools({
    ...
    const workspaceTools = chunkNWC75FLU_cjs.createWorkspaceTools(workspace);
  ...
  async listSkillTools({
    ...
    const skillTools = chunkNWC75FLU_cjs.createSkillTools(workspace.skills);
  ...
  async listAssignedTools({
    ...
    const assignedCoreToolEntries = await Promise.all(
      assignedToolEntries.map(async ([k, tool2]) => {
        ...
        return [k, chunkDDIY3CRP_cjs.makeCoreTool(tool2, options, void 0, autoResumeSuspendedTools)];
      })
    );
    ...
    if (toolsFromToolsets.length > 0) {
      ...
      for (const toolset of toolsFromToolsets) {
        for (const [toolName, tool2] of Object.entries(toolset)) {
          ...
          const convertedToCoreTool = chunkDDIY3CRP_cjs.makeCoreTool(toolObj, options, "toolset", autoResumeSuspendedTools);
  ...
  async listClientTools({
    ...
    if (clientToolsForInput.length > 0) {
      ...
      for (const [toolName, tool2] of clientToolsForInput) {
        ...
        const convertedToCoreTool = chunkDDIY3CRP_cjs.makeCoreTool(toolRest, options, "client-tool", autoResumeSuspendedTools);
        toolsForRequest[toolName] = convertedToCoreTool;
      }
    }
    return toolsForRequest;
...
${effectiveInstructions}`;
            ...
            try {
              ...
              if ((methodType === "generate" || methodType === "generateLegacy") && supportedLanguageModelSpecifications.includes(modelVersion)) {
                const generateResult = resumeData ? await agent.resumeGenerate(resumeData, {
                  ...
                  requestContext,
                  ...chunkLIRWMGGI_cjs.resolveObservabilityContext(context ?? {}),
        ...
        convertedAgentTools[`agent-${agentName}`] = chunkDDIY3CRP_cjs.makeCoreTool(
          toolObj,
          options,
          void 0,
          autoResumeSuspendedTools
  ...
  async listWorkflowTools({
    ...
    if (Object.keys(workflows).length > 0) {
      for (const [workflowName, workflow] of Object.entries(workflows)) {
        ...
        convertedWorkflowTools[`workflow-${workflowName}`] = chunkDDIY3CRP_cjs.makeCoreTool(
          toolObj,
          options,
          void 0,
          autoResumeSuspendedTools
  ...
  async convertTools({
    ...
    if (this.#mastra) {
      mastraProxy = chunkDDIY3CRP_cjs.createMastraProxy({ mastra: this.#mastra, logger });
    }
  ...
  async generate(messages, options) {
    ...
    const mergedOptions = chunkDDIY3CRP_cjs.deepMerge(
      defaultOptions,
      options ?? {}
    );
  ...
  async resumeStream(resumeData, streamOptions) {
    ...
    let mergedStreamOptions = chunkDDIY3CRP_cjs.deepMerge(
      defaultOptions,
      streamOptions ?? {}
    );
    const llm = await this.getLLM({
  ...
  getEncoder() {
    if (!this.encoderPromise) {
      if (this.customEncoding) {
        ...
        this.encoderPromise = chunkNWC75FLU_cjs.getTiktoken();
      }
    }
...
var ToolSearchProcessor = class {
  ...
  constructor(options) {
    ...
    this.bm25Index = new chunkNWC75FLU_cjs.BM25Index({}, TOOL_SEARCH_TOKENIZE_OPTIONS);
    this.indexTools();
    if (this.ttl > 0) {
...
`;
  ...
  generateEmptyFromSchemaInternal(schema) {
    const result = chunkDDIY3CRP_cjs.generateEmptyFromSchema(schema);
...
var SemanticRecall = class {
  ...
  formatCrossThreadMessages(messages, currentThreadId) {
    let result = ``;
    const v1Messages = new chunkNN4C4AVA_cjs.MessageList().add(messages, "memory").get.all.v1();
    let lastYmd = null;
...
https://mastra.ai/en/docs/memory/overview`
  ...
  getMergedThreadConfig(config) {
    ...
    const mergedConfig = chunkDDIY3CRP_cjs.deepMerge(this.threadConfig, config || {});
...
var MockMemory = class extends MastraMemory {
  constructor({
    ...
    super({
      name: "mock",
      storage: storage || new chunkK5H3MT3B_cjs.InMemoryStore(),
      options: {
