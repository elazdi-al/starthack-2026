...
var chunkEQ7RFQAH_cjs = require('./chunk-EQ7RFQAH.cjs');
var chunk5INB3RBA_cjs = require('./chunk-5INB3RBA.cjs');
var chunkXYDSWAZO_cjs = require('./chunk-XYDSWAZO.cjs');
var chunkP7FGMAQE_cjs = require('./chunk-P7FGMAQE.cjs');
...
var Mastra = class {
  ...
  get datasets() {
    if (!this.#datasets) {
      this.#datasets = new chunkXYDSWAZO_cjs.DatasetsManager(this);
    }
  ...
  constructor(config) {
    ...
    const workflowEventProcessor = new chunkEQ7RFQAH_cjs.WorkflowEventProcessor({ mastra: this });
    ...
    if (storage) {
      storage = chunkFSGRQN2S_cjs.augmentWithInit(storage);
    }
    if (config?.observability) {
      if (typeof config.observability.getDefaultInstance === "function") {
        ...
        this.#observability = new chunkLIRWMGGI_cjs.NoOpObservability();
      }
    } else {
      this.#observability = new chunkLIRWMGGI_cjs.NoOpObservability();
    }
    ...
    chunkMR7ZWBL6_cjs.registerHook("onScorerRun" /* ON_SCORER_RUN */, chunkP7FGMAQE_cjs.createOnScorerHook(this));
  ...
  addAgent(agent, key, options) {
    ...
    let mastraAgent;
    if (chunk5INB3RBA_cjs.isToolLoopAgentLike(agent)) {
      mastraAgent = chunk5INB3RBA_cjs.toolLoopAgentToMastraAgent(agent, { fallbackName: key });
    } else {
      mastraAgent = agent;
    }
   ...
   */
  setStorage(storage) {
    this.#storage = chunkFSGRQN2S_cjs.augmentWithInit(storage);
  }
  setLogger({ logger }) {
    this.#logger = logger;
    if (this.#agents) {
   ...
   */
  get loggerVNext() {
    return this.#observability.getDefaultInstance()?.getLoggerContext?.() ?? chunkLIRWMGGI_cjs.noOpLoggerContext;
  }
  /**
   ...
   */
  get metrics() {
    return this.#observability.getDefaultInstance()?.getMetricsContext?.() ?? chunkLIRWMGGI_cjs.noOpMetricsContext;
  }
  getServerMiddleware() {
  ...
  #syncGatewayRegistry() {
    try {
      ...
      import('./provider-registry-MKHIXI4D.cjs').then(async ({ GatewayRegistry }) => {
...
//# sourceMappingURL=chunk-PEPNSYRB.cjs.map
//# sourceMappingURL=chunk-PEPNSYRB.cjs.map